...
def get_correct_schema_rate(
    ...
    stag_cate = "use_stag" if use_stag else "no_stag"
    ideal_func_name = {}
    for entry in gorilla:
        ideal_func_name[entry["id"]] = set()
        for tool in entry["tool"]:
            ideal_func_name[entry["id"]].add(tool["function"]["name"])

    for entry in summary:
        if '{"name":' in entry[stag_cate]["output"]:
            if not entry[stag_cate]["success"]:
                err_type = entry[stag_cate]["err_type"]
                if err_type == "PARA_VALUE_ERROR":
                    correct_schema += 1
                elif (
                    err_type == "FUNC_SELECT_ERROR"
                    or err_type == "FUNC_NAME_ERROR"
                    or err_type == "CALL_NUMBER_ERROR"
                ):
                    flag = True
                    for call in entry[stag_cate]["call"]:
                        if (
                            call["function"]["name"]
                            not in ideal_func_name[entry["id"]]
                        ):
                            flag = False
                    if flag:
                        correct_schema += 1
