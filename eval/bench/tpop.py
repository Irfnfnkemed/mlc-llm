...
def check_acc(
    args: argparse.argparse.Namespace, dataset: GorillaDataset, output_dir: str
):
    ...
    if args.dataset == "BFCL_v3_simple" or args.dataset == "BFCL_v3_live_simple":
        for request in request_records:
            ...
            if len(request["call"]) != 1:
                ...
                format, call, err = dataset.check_simple(
                    request["call"][0], info["tool"][0], info["ideal_call"][0]
                )
            if not format:
                final_output["fail_format"].append(request["id"])
            if not call:
            ...
            if len(request["call"]) != 1:
                ...
                for tool in info["tool"]:
                    if tool["function"]["name"] == info["ideal_call"][0]["name"]:
                        ...
                        break
                format, call, err = dataset.check_simple(
                    request["call"][0], expected_tool, info["ideal_call"][0]
                )
            if not format:
                final_output["fail_format"].append(request["id"])
            if not call:
            ...
            if len(request["call"]) != len(info["ideal_call"][0]):
                ...
                for ideal in info["ideal_call"]:
                    ...
                    if expected_request is None:
                        ...
                        format, call, err = dataset.check_simple(
                            expected_request, expected_tool, ideal
                        )
    ...
    print(
        f"correct_format: {correct_format}/{count}, correct_call: {correct_call}/{count}"
    )
...
def main(args: argparse.argparse.Namespace):
    ...
    def _main():
        ...
        tpop_file = f"{args.output_root}/tpop.json"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)

        for i, pipeline in enumerate(pipelines):
            report, request_records = run_pipeline(pipeline, dataset, args)
            with open(tpop_file, "r") as f:
                tpop = json.load(f)
            if os.path.basename(args.tokenizer) not in tpop:
                tpop[os.path.basename(args.tokenizer)] = {}
            tpop_sum = 0
            for request in request_records:
                tpop_sum += request.metrics.time_to_first_token_s
            ...
            tpop[os.path.basename(args.tokenizer)][
                "use_stag" if args.use_stag else "no_stag"
            ] = tpop_sum / len(request_records)
            with open(tpop_file, "w") as f:
                json.dump(tpop, f, indent=4)
            exec_feature = (
        ...
        if args.debug_dump:
            debug_dump_filepath = (
                f"{output_dir}/bench.csv"[:-4]
                if f"{output_dir}/bench.csv".endswith(".csv")
                else f"{output_dir}/bench.csv"
            ) + "_debug_dump.log"
